...
namespace PixivWPF.Common
{
    ...
    public class DownloadInfo : INotifyPropertyChanged, IDisposable
    {
        ...
        public bool IsStart
        {
            get { return (Instance?.IsDownloading ?? default); }
            set
            {
                if (value) Start();
            ...
            if (Instance is DownloadItem)
            {
                try
                {
                    await new Action(() =>
                    {
                        ...
                        if (!IsStart) Instance?.Start(setting.DownloadWithFailResume);
                    }).InvokeAsync();
                }
        ...
        {
            SaveAsJPEG = on ?? false;
            Instance?.PART_SaveAsJPEG.IsOn = SaveAsJPEG;
            Instance?.miSaveAsJPEG.IsChecked = SaveAsJPEG;
        }
    ...
    public partial class DownloadItem : UserControl, INotifyPropertyChanged, IDisposable
    {
        ...
        {
            get { return (Info?.Canceled ?? false); }
            set { Info?.Canceled = value; }
        }

        public string FailReason
        {
            get { return (Info?.FailReason ?? string.Empty); }
            set { Info?.FailReason = value; }
        }

        public string ConvertReason
        {
            get { return (Info?.ConvertReason ?? string.Empty); }
            set { Info?.ConvertReason = value; }
        }

        public string Url
        {
            get { return (Info?.Url ?? string.Empty); }
            set { Info?.Url = value; }
        }
        ...
        {
            get { return (Info?.JPEGQuality ?? setting.DownloadConvertJpegQuality); }
            set { Info?.JPEGQuality = value; }
        }
        ...
        public string ThumbnailUrl
        {
            get { return (Info?.ThumbnailUrl ?? string.Empty); }
            set { Info?.ThumbnailUrl = value; }
        }

        public string FileName
        {
            get { return (Info?.FileName ?? string.Empty); }
            set
            {
                Info?.FileName = value;
        ...
        {
            get { return (!string.IsNullOrEmpty(Info?.FileName) ? Path.GetDirectoryName(Info?.FileName) : string.Empty); }
        }

        [DefaultValue(true)]
        public bool AutoStart
        {
            get { return (Info?.AutoStart ?? default); }
        ...
        public bool IsStart
        {
            get { return (Info?.IsStart ?? default); }
            set { Info?.IsStart = value; }
        }

        public Tuple<double, double> Progress
        {
            get { return (Info?.Progress ?? new Tuple<double, double>(Received, Length)); }
        }

        public long Received
        {
            get { return (Info?.Received >= 0 ? Info?.Received ?? 0 : 0); }
            set { Info?.Received = value; }
        }

        public long Length
        {
            get { return (Info?.Length >= 0 ? Info?.Length ?? 0 : 0); }
            set { Info?.Length = value; }
        }

        public DateTime LastModified
        {
            get { return (Info?.LastModified ?? default); }
            set { Info?.LastModified = value; }
        }

        [DefaultValue(true)]
        public bool Overwrite
        {
            get { return (Info?.Overwrite ?? default); }
        ...
        public bool SingleFile
        {
            get { return (Info?.SingleFile ?? default); }
            set { Info?.SingleFile = value; }
        }

        public DateTime FileTime
        {
            get { return (Info?.FileTime ?? default); }
            set { Info?.FileTime = value; }
        }

        [DefaultValue(DownloadItemState.Unknown)]
        public DownloadItemState State
        {
            get { return (Info?.State ?? default); }
            set
            {
                Info?.State = value;
                NotifyPropertyChanged("StateChanged");
        ...
        public bool IsIdle
        {
            get { return (State == DownloadItemState.Idle); }
        }

        [DefaultValue(false)]
        public bool IsPaused
        {
            get { return (State == DownloadItemState.Paused); }
        }

        [DefaultValue(false)]
        public bool IsFailed
        {
            get { return (State == DownloadItemState.Failed); }
        }

        [DefaultValue(false)]
        public bool IsFinished
        {
            get { return (State == DownloadItemState.Finished); }
        }

        [DefaultValue(false)]
        public bool IsCanceling
        {
            get { return ((cancelSource?.IsCancellationRequested ?? default) || Canceling); }
        }
        ...
        public bool CanDownload
        {
            get
            {
                return (State == DownloadItemState.Idle || State == DownloadItemState.Paused || State == DownloadItemState.Failed || State == DownloadItemState.NonExists || State == DownloadItemState.Finished || Downloading?.CurrentCount > 0);
            }
        }
        ...
        {
            get { return (Info?.StartTime ?? default); }
            set { Info?.StartTime = value; }
        }
        private DateTime EndTick
        {
            get { return (Info?.EndTime ?? default); }
            set { Info?.EndTime = value > DateTime.Now ? DateTime.Now : value; }
        }
        private TimeSpan TotalElapsed
        {
            get { return (Info?.TotalElapsed ?? default); }
            set { Info?.TotalElapsed = value; }
        }
        private TimeSpan LastTotalElapsed
        {
            get { return (Info?.LastTotalElapsed ?? default); }
            set { Info?.LastTotalElapsed = value; }
        }
        private TimeSpan LastElapsed
        {
            get { return (Info?.LastElapsed ?? default); }
            set { Info?.LastElapsed = value; }
        }
        ...
        private void CheckProperties()
        {
            if (Tag is DownloadInfo)
            {
                try
                {
                    ...
                    else if (State == DownloadItemState.Paused)
                    {
                        AutoStart = false;
                        miRemove.IsEnabled = true;
                        miStopDownload.IsEnabled = false;
        ...
        {
            Dispatcher.InvokeAsync(() => CheckProperties());
        }
            ...
            try
            {
                await new Action(() =>
                {
                    ...
                    if (SaveAsJPEG)
                    {
                        ...
                        if (ret is byte[] && ret.Length > 16)
                        {
                            if (ret.Length >= bytes.Length)
                                ...
                                ret_save = FileName.WriteAllBytes(ret);
                        }
                    }
                    else
                        ret_save = FileName.WriteAllBytes(bytes);
                    ...
                    if (ret_save)
                    {
                        ...
                        catch (Exception exa) { exa.ERROR($"SetFileTime_{Path.GetFileNameWithoutExtension(FileName)}"); }
        ...
        private async Task<string> SaveFile(string FileName, string source)
        {
            ...
            try
            {
                ...
                {
                    result = await SaveFile(FileName, File.ReadAllBytes(source));
                }
            }
            catch (Exception ex)
            {
        ...
        public async void Start(bool continuation = true, bool restart = false)
        {
            ...
            if (CanDownload && Downloading?.CurrentCount <= 0) Downloading?.Release();
            ...
            else
            {
                if (CanDownload || restart)
                {
                    ...
                    await Task.Delay(Application.Current.Random(50, 500)).ContinueWith(async t =>
                    {
                        this.DoEvents();
        ...
        private async Task Cancel()
        {
            if (!IsCanceling)
            {
                ...
                catch (Exception ex) { ex.ERROR($"{this.Name ?? GetType().Name}_HttpRequest_Cancel"); }
                finally
                {
            ...
            try
            {
                ...
                if (AutoStart)
                {
                    ...
                    else if (State == DownloadItemState.Idle) //|| State == DownloadState.Failed)
                    {
                        ...
                        if (!IsDownloading) Start(setting.DownloadWithFailResume);
                    }
                }
            ...
            else if (sender == PART_SaveAsJPEG)
            {
                Action<DownloadInfo> action = (info) =>
                {
                    if (info is DownloadInfo)
                    {
                        ...
                        info.Instance?.PART_SaveAsJPEG.IsOn = info.SaveAsJPEG;
                        info.Instance?.PART_SaveAsJPEG.IsEnabled = info.State != DownloadItemState.Finished;

                        info.Instance?.miSaveAsJPEG.IsChecked = info.SaveAsJPEG;
                        info.Instance?.miSaveAsJPEG.IsEnabled = info.State != DownloadItemState.Finished;
                    }
                };
                if (!IsEnabled || !multiple) action.Invoke(Info);
                else Commands.RunDownloadItemAction.Execute(action);
