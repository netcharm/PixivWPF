...
#pragma warning disable CA1822
    ...
    public class Magick
    {
        private const int QUALITY_UNDEFINED = 0;
        private const int MAX_QUALITY = 100;

        #region Hash/Sum Tables
        private static int[] hash = new int[]
        {
            1020, 1015, 932, 848, 780, 735, 702, 679, 660, 645, 632, 623, 613, 607, 600, 594,
        ...
        {
            32640, 32635, 32266, 31495, 30665, 29804, 29146, 28599, 28104, 27670, 27225, 26725,
        ...
        private int[] singlehash = new int[]
        {
        ...
        private int[] singlesums = new int[]
        {
        ...
        public int GetJPEGImageQuality(QuantTables qts)
        {
            var result = QUALITY_UNDEFINED;
            if (qts is QuantTables && qts.HasData())
            {
        ...
        public int GetJPEGImageQuality(Stream source)
        {
            var result = QUALITY_UNDEFINED;
            if (source is Stream && source.CanRead)
            {
                QuantTables qts = new QuantTables();
                qts.GetDataFromStream(source);
                result = GetJPEGImageQuality(qts);
            }
            return (result);
        }

        public int GetJPEGImageQuality(string file)
        {
            var result = QUALITY_UNDEFINED;
            if (File.Exists(file))
            {
                result = GetJPEGImageQuality(file);
            }
            return (result);
        }

        public int GetJPEGImageQuality(FileInfo file)
        {
            var result = QUALITY_UNDEFINED;
            if (!string.IsNullOrEmpty(file.FullName) && file.Exists)
                result = GetJPEGImageQuality(file.FullName);
            return (result);
        }
    ...
    {
        public const int MAX_TABLE_COUNT = 2;
        public const int TABLE_LENGTH = 64;

        private int[][] tables = new int[MAX_TABLE_COUNT][] { new int[TABLE_LENGTH], new int[TABLE_LENGTH] };
        private byte[] tablesInitFlag = new byte[MAX_TABLE_COUNT];

        private static byte JPEG_FLAG_START =  0xff;
        private static byte JPEG_FLAG_DQT =  0xdb;// define quantization  table

        private static byte[] JPEG_HEADER_FLAG = new byte[] { (byte) 0xff, (byte) 0xd8 };

        public void GetDataFromFile(FileInfo file)
        {
            if (file.Exists)
            {
                using (var fs = new FileStream(file.FullName, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    GetDataFromStream(fs);
                }
            }
        }
        ...
        public void GetDataFromStream(Stream source)
        {
            try
            {
                if (source is Stream && source.CanRead)
                {
                    ...
                    while (len > index)
                    {
                        ...
                        if (buf[index + 1] == JPEG_FLAG_DQT)
                        {// it's a begin of DQT
                            buf = new byte[sectionLength - 2];// dqt 长度不超过4个表长，即不超过4*64再加一些标志位
                            len = source.Read(buf, 0, buf.Length);
                        ...
                        {
                            source.Position = source.Position + (sectionLength - 2);
                            len = source.Read(buf, 0, 4);
                            index = 0;
                            continue;
                        }
                    }
    ...
    public class ExifData
    {
        ...
        {
            TagItem t;
            if (GetTagItem(TagSpec, out t) && ((uint)ValueCount <= MaxTagValueCount))
            {
        ...
        private void ReadJepg()
        {
            ...
            do
            {
                ...
                ReadJpegBlock(SourceExifStream, BlockContent, out BlockContentSize, out BlockMarker, out BlockType, out MetaDataIndex);
                if (BlockType != ImageFileBlock.Unknown)
                {
                    if ((BlockType == ImageFileBlock.Exif) && (!ExifBlockFound))
                    {
                        ...
                        int IfdPrimaryDataOffset;
                        EvaluateTiffHeader(SourceExifBlock, TiffHeaderAndExifBlockLen, out IfdPrimaryDataOffset);
        ...
        private void ReadTiff()
        {
            ...
            int IfdPrimaryDataOffset;
            EvaluateTiffHeader(BlockContent, TiffHeaderBytesRead, out IfdPrimaryDataOffset);
            EvaluateExifBlock(IfdPrimaryDataOffset);
        }
        ...
        private void ReadPng()
        {
            ...
            {
                int DataLength;
                ReadPngBlockHeader(SourceExifStream, TempData, out DataLength, out ChunkType);
                ImageFileBlock BlockType;
                ...
                if (BlockType != ImageFileBlock.Unknown)
                {
                    if ((BlockType == ImageFileBlock.Exif) && (!ExifBlockFound))
                    {
                        ...
                        int IfdPrimaryDataOffset;
                        EvaluateTiffHeader(SourceExifBlock, DataLength, out IfdPrimaryDataOffset);
                        EvaluateExifBlock(IfdPrimaryDataOffset);
          ...
          out ImageFileBlock BlockType, out int MetaDataIndex)
        {
            ...
            MetaDataIndex = 0;
            int ContentSize;
            ReadJpegBlockMarker(ImageStream, out BlockMarker, out ContentSize);
            if (ContentSize > 0)
            {
        ...
        private void SaveJpeg(Stream SourceStream, Stream DestStream)
        {
            ...
            {
                int BlockContentSize;
                ReadJpegBlockMarker(SourceStream, out BlockMarker, out BlockContentSize);
            ...
            // Write new EXIF block if the new EXIF block is not 
            FlexArray NewExifBlock;
            CreateExifBlock(out NewExifBlock, TiffHeaderLen);
            ...
            else if (NewExifBlockLen > 0)
            {
                ...
                byte[] TiffHeader;
                CreateTiffHeader(out TiffHeader, TiffHeaderLen);
            ...
            do
            {
                ...
                ImageFileBlock BlockType;
                int _;
                ReadJpegBlock(SourceStream, BlockContent, out BlockContentSize, out BlockMarker, out BlockType, out _);
                if (BlockMarker == JpegSosMarker)
                {
        ...
        private void SaveTiff(Stream SourceStream, Stream DestStream)
        {
            ...
            do
            {
                ...
                CurrentImageExif = CreateSubExifData(SourceStream, ref SourceStreamExifBlockOffset, SourceStreamByteOrder,
                  out SegmentOffsetTable, out SegmentByteCountTable,
                  out SegmentOffsetsTag, out SegmentByteCountsTag);
        ...
        private void SavePng(Stream SourceStream, Stream DestStream)
        {
            ...
            do
            {
                ...
                int DataLength;
                ReadPngBlockHeader(SourceStream, TempData, out DataLength, out ChunkType);
                ImageFileBlock BlockType;
                DetectPngImageBlock(SourceStream, BlockContent, ChunkType, out BlockType);
                if (BlockType != ImageFileBlock.Unknown)
                {
        ...
        private int CreateExifBlock(out FlexArray NewExifBlock, int CurrentExifBlockOffset)
        {
            ...
            int WriteIndex = 0;
            TagItem PrivateDataPointerTag;
            TagItem GpsInfoDataPointerTag;
            TagItem InteroperabilityPointerTag;
            UpdateIfdPointerTags(out PrivateDataPointerTag, out GpsInfoDataPointerTag, out InteroperabilityPointerTag);
            int PrivateDataIfdPointerIndex;
            int GpsInfoDataIfdPointerIndex;
            int ThumbnailDataIfdPointerIndex;
            CreateIfdPrimaryData(WriteExifBlock, ref WriteIndex, CurrentExifBlockOffset, out PrivateDataIfdPointerIndex,
              out GpsInfoDataIfdPointerIndex, out ThumbnailDataIfdPointerIndex);
            int InteroperabilityIfdPointerIndex;
            CreateIfdPrivateData(WriteExifBlock, ref WriteIndex, CurrentExifBlockOffset, PrivateDataIfdPointerIndex, PrivateDataPointerTag,
              out InteroperabilityIfdPointerIndex);
        ...
        private bool GetDateAndTimeWithMillisecHelper(out DateTime Value, ExifTag DateAndTimeTag, ExifTag MillisecTag)
        {
            ...
            if (GetTagValue(DateAndTimeTag, out Value))
            {
                Success = true;
                string SubSec;
                if (GetTagValue(MillisecTag, out SubSec, StrCoding.Utf8))
                {
                    string s = SubSec;
                    int len = s.Length;
                    if (len > 3) s = s.Substring(0, 3);
                    int MilliSec;
                    if (int.TryParse(s, out MilliSec) && (MilliSec >= 0))
                    {
    ...
    public enum ExifTag
    {
        ...
        JPEGQuality = (ExifIfd.PrimaryData << ExifData.IfdShift) | ExifTagId.JPEGQuality,
        JPEGQTables = (ExifIfd.PrimaryData << ExifData.IfdShift) | ExifTagId.JPEGQTables,
        JPEGDCTables = (ExifIfd.PrimaryData << ExifData.IfdShift) | ExifTagId.JPEGDCTables,
        JPEGACTables = (ExifIfd.PrimaryData << ExifData.IfdShift) | ExifTagId.JPEGACTables,

        LuminanceTable = (ExifIfd.PrimaryData << ExifData.IfdShift) | ExifTagId.LuminanceTable,
        ChrominanceTable = (ExifIfd.PrimaryData << ExifData.IfdShift) | ExifTagId.ChrominanceTable,
