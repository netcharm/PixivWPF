    ...
    public partial class App : Application
    {
        ...
        private static void ReportMessage(Exception ex)
        {
            Current?.Dispatcher.Invoke(() =>
            {
                ...
                {
                    (Current?.MainWindow as MainWindow).ReportMessage(ex);
                }
            });
        }
        ...
        private void PipeReceiveData(IAsyncResult result)
        {
            try
            {
                if (!_pipOnClosing_ && result is not null && result.IsCompleted)
                {
                    NamedPipeServerStream? ps = result.AsyncState as NamedPipeServerStream;
                    ...
                    if (ps.CanRead)
                    {
                        ...
                        {
                            var content = Newtonsoft.Json.JsonConvert.DeserializeObject<NamedPipeContent>(contents);
